...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($do === 'delete') {
        $delId = int_param('id');
        Database::run('UPDATE gallery_items SET category_id = NULL WHERE category_id = ?', [$delId]);
        Database::delete('gallery_categories', $delId);
        flash_set('success', 'Kategori silindi (görseller kategorisiz bırakıldı).');
        admin_redirect('gallery-categories.php');
    }
    ...
    if ($do === 'save') {
        ...
        $data['slug'] = unique_slug('gallery_categories', $slug, $data['lang'], $id);

        if ($id) {
            Database::update('gallery_categories', $id, $data);
            flash_set('success', 'Kategori güncellendi.');
        } else {
            Database::insert('gallery_categories', $data);
...
$edit = $id ? Database::row('SELECT * FROM gallery_categories WHERE id = ?', [$id]) : null;
$rows = Database::all('SELECT c.*, (SELECT COUNT(*) FROM gallery_items g WHERE g.category_id = c.id) AS item_count FROM gallery_categories c ORDER BY lang, sort_order');

$pageTitle = 'Galeri Kategorileri';
...
<div class="two-col">
    <div class="panel">
        ...
        <form method="post" action="<?= e(admin_url('gallery-categories.php', $edit ? ['id' => $edit['id']] : [])) ?>">
            <?= Csrf::field() ?>
            ...
            <div class="form-actions">
                ...
                <?php if ($edit): ?><a class="btn btn-ghost" href="<?= e(admin_url('gallery-categories.php')) ?>">Vazgeç</a><?php endif; ?>
            </div>
        </form>
    </div>
    <div class="table-wrap">
        <table>
            <thead><tr><th>Ad</th><th>Slug</th><th>Dil</th><th>Görsel</th><th>Sıra</th><th>Durum</th><th></th></tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div class="row-actions">
                            <a class="btn btn-sm btn-ghost" href="<?= e(admin_url('gallery-categories.php', ['id' => $r['id']])) ?>"><?= icon('edit', 'icon-sm') ?> Düzenle</a>
                            <form method="post" action="<?= e(admin_url('gallery-categories.php')) ?>" data-confirm="Kategori silinsin mi?">
                                <?= Csrf::field() ?>
